...
import { updatePlantApi } from "../../api/explorePlantApi";
import { uploadImageToCloudinary } from "../../lib/uploadImageToCloudinary";

export default function UpdatePlantModal({ isOpen, onClose, plant }) {
  const queryClient = useQueryClient();

  const { mutate, isPending } = useMutation({
    mutationFn: updatePlantApi,
    onSuccess: () => {
  ...
  useEffect(() => {
    if (plant && isOpen) {
      setFormData({
        ...
        ims_url: plant.ims_url,
        type: plant.type,
        plantSizes: plant.plantSizes || [{ size: "", price: "" }],
      });
      setImagePreview(plant.ims_url);
    }
  }, [plant, isOpen]);
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-2xl">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-800">Update Plant</h2>
            <button
              onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {!imagePreview ? (
              ...
              <div className="relative">
                ...
                <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs">
                  ...
                  {formData.ims_url ? "Uploaded" : "Current image"}
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
              placeholder="e.g., flowering, indoor, outdoor"
            />
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isPending ? "Updating..." : "Update Plant"}
            </button>
          </div>
        </form>
      </div>
